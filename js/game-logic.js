...
// نکته مهم: آمار تانک (move/range/damage) روی خود تانک ذخیره نمیشه، از TANK_TYPES خونده میشه
function getValidMoveCells(state, tank) {
  const conf = TANK_TYPES[tank.type];
  const results = [];
  for (const [dx, dy] of DIRS) {
    for (let step = 1; step <= conf.move; step++) {
...
function getValidShootCells(state, tank) {
  ...
  const results = [];
  for (const [dx, dy] of DIRS) {
    for (let step = 1; step <= conf.range; step++) {
      const x = tank.x + dx * step;
      const y = tank.y + dy * step;
      if (!inBounds(x, y)) break;
...
function applyMove(state, tankId, x, y) {
  ...
  const dx = Math.sign(x - tank.x);
  const dy = Math.sign(y - tank.y);
  if (dx !== 0 || dy !== 0) tank.facing = { dx, dy };
  tank.x = x;
  tank.y = y;
  tank.used = true;
  state.lastEffect = { type: 'move', tankId, ts: Date.now() };
}
...
function applyShoot(state, tankId, x, y) {
  ...
  state.lastEffect = { type: 'shoot', tankId, fromX: tank.x, fromY: tank.y, x, y, ts: Date.now() };
  return target || null;
}
...
function buildTanksFromPlacement(owner, placedList) {
  // بازیکن ۱ رو به سمت پایین (جنوب) و بازیکن ۲ رو به سمت بالا (شمال) می‌چرخونیم
  // که یعنی از ابتدا تانک‌ها رو به روی حریف باشن
  const defaultFacing = owner === 'player1' ? { dx: 0, dy: 1 } : { dx: 0, dy: -1 };
